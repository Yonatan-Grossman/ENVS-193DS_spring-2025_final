...
```{r}
nest_boxes$predicted.sp <- predict(model1, type = "response")

predictions <- predict(model1, newdata = nest_boxes, type = "link", se.fit = TRUE)

fit <- predictions$fit
se <- predictions$se.fit

nest_boxes$fit <- fit
nest_boxes$ci_lower_pre <- fit - 1.96 * se
nest_boxes$ci_upper_pre <- fit + 1.96 * se


nest_boxes$predicted.sp <- plogis(fit)
nest_boxes$ci.lower <- plogis(nest_boxes$ci_lower_pre)
nest_boxes$ci.upper <- plogis(nest_boxes$ci_upper_pre)
...
ggplot(nest_boxes,
       aes(x = edge.distance,
           ...
           color = season)) +
  geom_line() +
  geom_jitter(aes(y = sp, color = season, shape = season), height = .05, width = 0) +
